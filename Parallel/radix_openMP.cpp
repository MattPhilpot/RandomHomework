...
int main()
{
	...
	cout << "Welcome to Philpot's Radix Sort\n\nWould you like to 'import' or generate a 'random' list? ";
	while(true)
	{
		cin >> input;
		if(input.compare("import") == 0)
		{
			cout << "Please name a file to import: ";
			cin >> input;
			cout << "\nImporting " << input << "..." << endl;
			infile.open(input);

			/*	Get Number of Integers in File for Dynamically Sized Integer Array	*/
			if(infile.is_open())
			{
				while(infile >> x)
				{
			...
			queue = new int[new_max];	/*	and Queue	*/
			infile.open(input);

			/*	Reading integers into _perfectly_ allocated array. Trying to be memory efficient	*/
			if(infile.is_open())
			{
				while(infile >> x)
				{
		...
		{
			cout << "Command Parse Error. Please Enter 'import' or 'random': ";
		}
	}
...
/*	Radix Sort. Where most radix sorts call a recurring function, I avoid calling functions altogether to save on time, space, and overhead.	*/
	for(n = 10; n <= digits; n*=10) /*	Determines which position will be sorted. ie. 1's, 10's 100's, etc.	*/
	{
		...
		{
			#pragma omp parallel for	/*	Automatically determines # of threads, and splits for-loop accordingly	*/
